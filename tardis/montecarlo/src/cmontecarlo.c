        ...
        int64_t imin, int64_t imax, int64_t * result)
{
    ...
    tardis_error_t
line_search (const double *x, double x_insert, int64_t number_of_lines,
        int64_t * result)
{
    tardis_error_t ret_val = TARDIS_ERROR_OK;
    int64_t imin = 0;
    ...
    else if (x_insert < x[imax])
    {
        *result = imax + 1;
    }
    else
    {
        ret_val = reverse_binary_search (x, x_insert, imin, imax, result);
        *result = *result + 1;
    }
    return ret_val;
}
        ...
        int64_t virtual_mode, rk_state *mt_state)
{
    ...
    if (virtual_mode == 0)
    {
        return montecarlo_one_packet_loop (storage, packet, 0, mt_state);
    }
    else
    {
...
#pragma omp critical
                {
                    ...
                    // FIXME: In the following code virt_packet and packet is mixed. Intentional?
        ...
        double distance, rk_state *mt_state)
{
    ...
    else if (rpacket_get_tau_event (packet) < tau_combined)
    {
        ...
        storage->last_line_interaction_shell_id[rpacket_get_id (packet)] =
            rpacket_get_current_shell_id (packet);
        storage->last_interaction_type[rpacket_get_id (packet)] = 2; //shouldn't enum be used here?
